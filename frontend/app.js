...
let adminState = { events: [], knowledge: [], members: [] };
const memberPaging = {
  memberList: { page: 1, pageSize: 24, items: [] },
  adminMemberList: { page: 1, pageSize: 12, items: [] },
};
...
function contentId(item) {
  return item.id || item.event_id || item.post_id || item.member_id || '';
}
...
function renderMembers(items, admin = false) {
  ...
  return rows.map((a) => `
    <article class="item member-item">
      ...
      ${admin ? renderAdminActions('alumni', a.member_id, disableActions) : ''}
    </article>
  `).join('');
}

function memberPageSize(target) {
  return target === 'adminMemberList' ? 12 : 24;
}

function memberTargetLabel(target) {
  return target === 'adminMemberList' ? 'admin member records' : 'member profiles';
}

function setMemberPage(target, pageNumber) {
  const state = memberPaging[target];
  if (!state) return;
  ...
  renderMembersPage(target);
}

function renderMembersPager(target, page, pageCount, total, from, to) {
  const label = memberTargetLabel(target);
  if (!total) return '';
  return `
    <div class="pagination" aria-label="${escapeHtml(label)} pagination">
      ...
      <div class="pagination-actions">
        <button type="button" class="secondary" data-member-page-target="${escapeHtml(target)}" data-member-page="first" ${page <= 1 ? 'disabled' : ''}>First</button>
        <button type="button" class="secondary" data-member-page-target="${escapeHtml(target)}" data-member-page="prev" ${page <= 1 ? 'disabled' : ''}>Previous</button>
        <span class="page-indicator">Page ${page} of ${pageCount}</span>
        <button type="button" class="secondary" data-member-page-target="${escapeHtml(target)}" data-member-page="next" ${page >= pageCount ? 'disabled' : ''}>Next</button>
        <button type="button" class="secondary" data-member-page-target="${escapeHtml(target)}" data-member-page="last" ${page >= pageCount ? 'disabled' : ''}>Last</button>
      </div>
    </div>
  `;
}

function renderMembersPage(target) {
  const state = memberPaging[target];
  if (!state) return;
  ...
  if (!total) {
    setHtml(target, renderMembers([], page === 'admin'));
    return;
  }
  ...
  setHtml(target, `${renderMembersPager(target, state.page, pageCount, total, from, to)}${renderMembers(pageItems, page === 'admin')}${renderMembersPager(target, state.page, pageCount, total, from, to)}`);
}
...
async function loadMembers(target = 'memberList') {
  if (!isLoggedIn()) {
    setHtml(target, '<p class="notice">Please sign in to search the member directory.</p>');
    return;
  }
  const data = await request(`/members?${memberParams().toString()}`);
  if (!data.success) {
    setHtml(target, `<p class="error">${escapeHtml(data.message || 'Unable to load members.')}</p>`);
    return;
  }

  if (!memberPaging[target]) memberPaging[target] = { page: 1, pageSize: memberPageSize(target), items: [] };
  memberPaging[target].items = Array.isArray(data.data) ? data.data : [];
  memberPaging[target].pageSize = memberPageSize(target);
  memberPaging[target].page = 1;
  renderMembersPage(target);
  if (page === 'admin') adminState.members = memberPaging[target].items;
}
...
async function saveMember() {
  ...
  setText('memberAdminStatus', data.message || '');
  if (data.success) {
    resetMemberForm();
    await refreshAdmin();
  }
...
async function deleteAdminItem(type, id) {
  if (!id) return;
  const routeType = type === 'members' ? 'members' : type;
...
function editMember(id) {
  const item = adminState.members.find((x) => x.member_id === id);
  if (!item) return;
  el('aId').value = id;
  ...
  setText('memberAdminStatus', `Editing ${item.full_name || id}`);
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
...
function updateAdminCounts() {
  setText('countMembers', adminState.members.length);
...
async function refreshAdmin() {
  ...
  const tasks = [loadEvents('adminEventList'), loadKnowledge('adminKnowledgeList'), loadMembers('adminMemberList')];
...
function wireEvents() {
  ...
  el('searchButton')?.addEventListener('click', () => loadMembers());
  el('saveContentButton')?.addEventListener('click', saveContent);
  el('resetContentButton')?.addEventListener('click', resetContentForm);
  el('saveMemberButton')?.addEventListener('click', saveMember);
  el('resetMemberButton')?.addEventListener('click', resetMemberForm);
  document.addEventListener('click', (event) => {
    ...
    const pageControl = event.target.closest('[data-member-page]');
    if (pageControl && !pageControl.disabled) {
      const target = pageControl.dataset.memberPageTarget;
      const state = memberPaging[target];
      if (!state) return;
      const pageCount = Math.max(1, Math.ceil(state.items.length / state.pageSize));
      const nextPage = {
        ...
        next: state.page + 1,
        last: pageCount,
      }[pageControl.dataset.memberPage];
      if (nextPage) setMemberPage(target, nextPage);
      return;
    }
    if (edit?.disabled || del?.disabled) return;
    if (edit) {
      const type = edit.dataset.edit;
      if (type === 'members') editMember(edit.dataset.id);
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/service-worker.js')
      .catch((error) => console.warn('VCNITY offline support unavailable:', error));
  });
}
